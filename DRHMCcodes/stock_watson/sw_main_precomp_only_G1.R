...
standta <- list(T=T,y=y,alpha=5.0,beta=0.5,zmean=0*y[1:(T-1)],xmean=0*y,lambdamean=0.0,lambdaprec= 5 + T - 1.5)
opts <- optimizing(drhmc.mdlt,data=standta,as_vector=FALSE)
inits = list(opts)
drhmc.0 <- sampling(drhmc.mdlt,data=standta,chains=1,seed=2,init=inits,control=list(adapt_delta=0.95))

xx <- summary(drhmc.0,"x")$summary
xmean <- xx[,"mean"]
zz <- summary(drhmc.0,"z")$summary
zmean <- zz[,"mean"]
lam <- summary(drhmc.0,"lambda")$summary
lammean <- lam[1]
lamprec <- 1.0/lam[3]^2

standta <- list(T=T,y=y,alpha=5.0,beta=0.5,zmean=0*zmean,xmean=0*xmean,lambdamean=lammean,lambdaprec=lamprec)

opts <- optimizing(drhmc.mdlt,data=standta,as_vector=FALSE)
inits <- list(opts,opts,opts,opts,opts,opts,opts,opts,opts,opts)
drhmc <- sampling(drhmc.mdlt,data=standta,chains=10,seed=2,init=inits,control=list(stepsize_jitter=0.5,metric="unit_e"))


summar <- (summary(drhmc,c("lambda","lambda_bar","z_bar[1]","x_bar[1]","tau_bar[1]","z[1]","x[1]","tau[1]"))$summary)

print(summar[,c(1,3,9)],digits=2)

print(mean(get_elapsed_time(drhmc)[,2]))

#pdf("sw_plot.pdf",width=7,height=12)
...
plot(time,tauf[,"mean"],col=2,type="l",lwd=2,xlab="", ylab=TeX("$\\tau$"),
...
plot(time,xx[,"mean"],col=2,type="l",lwd=2,xlab="", ylab=TeX("$\\mathbf{x}$"),
...
#pdf("sw_trace.pdf",width=7,height=12)
par(mfrow=c(4,1))
