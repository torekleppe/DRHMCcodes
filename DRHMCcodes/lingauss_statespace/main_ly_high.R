...
kern <- post_kern_ly(from=0,to=10,ng=1000,lam_x=lam_x,omega=omega,set1$y, pri_mean=lam_y)
...
pdf("ly_plot_high.pdf",width=14,height=7)
par(mfrow=c(2,4))
ts.plot(extract(hmc,c("lam_y"))$lam_y,ylab=TeX("$\\lambda_y$"),xlab="Iteration",main=TeX("prior rescaling"),xlim=c(0,10000),ylim=c(3.0,5.0))
for(i in 0:10){
    lines(c(i*1000,i*1000),c(0,100),col=2)
}
ts.plot(extract(drhmc.0,c("lam_y"))$lam_y,ylab=TeX("$\\lambda_y$"),xlab="Iteration",
main=TeX("DRHMC, $\\mu_{(2)} = 0$"),xlim=c(0,10000),ylim=c(3.0,5.0))
for(i in 0:10){
    lines(c(i*1000,i*1000),c(0,100),col=2)
}
ts.plot(extract(drhmc.y,c("lam_y"))$lam_y,ylab=TeX("$\\lambda_y$"),xlab="Iteration",main=TeX("DRHMC, $\\mu_{(2)} = \\mathbf{y}$"),xlim=c(0,10000),ylim=c(3.0,5.0))
for(i in 0:10){
    lines(c(i*1000,i*1000),c(0,100),col=2)
}

ts.plot(extract(drhmc.xcy,c("lam_y"))$lam_y,ylab=TeX("$\\lambda_y$"),xlab="Iteration",
main=TeX("DRHMC, $\\mu_{(2)}=E(\\mathbf{x}|\\mathbf{y},\\lambda_y)$"),xlim=c(0,10000),ylim=c(3.0,5.0))
for(i in 0:10){
    lines(c(i*1000,i*1000),c(0,100),col=2)
}


hist(extract(hmc,c("lam_y"))$lam_y,100,probability=T,main="",xlab=TeX("$\\lambda_y$"), xlim=c(3.0,5.0))
lines(x=kern[,1],y=kern[,2]/(kern[2,1]-kern[1,1]),col=2)
hist(extract(drhmc.0,c("lam_y"))$lam_y,100,probability=T,main="",xlab=TeX("$\\lambda_y$"), xlim=c(3.0,5.0))
lines(x=kern[,1],y=kern[,2]/(kern[2,1]-kern[1,1]),col=2)
hist(extract(drhmc.y,c("lam_y"))$lam_y,100,probability=T,main="",xlab=TeX("$\\lambda_y$"), xlim=c(3.0,5.0))
lines(x=kern[,1],y=kern[,2]/(kern[2,1]-kern[1,1]),col=2)
hist(extract(drhmc.xcy,c("lam_y"))$lam_y,100,probability=T,main="",xlab=TeX("$\\lambda_y$"), xlim=c(3.0,5.0))
lines(x=kern[,1],y=kern[,2]/(kern[2,1]-kern[1,1]),col=2)
dev.off()
